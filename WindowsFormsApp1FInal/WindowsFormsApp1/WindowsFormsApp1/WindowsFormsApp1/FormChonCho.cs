    ...
    public partial class FormChonCho : Form
    {
        ...
        private void ChonPhim()
        {
            ...
            for (int i =0; i< DataTablePhim.Rows.Count;i++)
            {
                ...
                pc.Image = Image.FromFile(DataTablePhim.Rows[i][8].ToString());
                pathanh = DataTablePhim.Rows[i][8].ToString(); 
        ...
        private void Button_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < DataTablePhim.Rows.Count; i++)
            {
                if (DataTablePhim.Rows[i][0].ToString() == r.Name)
                {
                    ...
                    ktsuat.pathAnh = DataTablePhim.Rows[i][8].ToString();
                }
            }
            ktsuat.ShowDialog();
        }
        ...
        public void luuVe()
        {
            ...
            foreach (ListViewItem item in lvThongTin.Items)
            {
                ...
                query = "Insert Into Ve Values ('"+ maVe +"','" + MaPhim + "','" + MASC + "','" + item.SubItems[1].Text + "','" + item.SubItems[2].Text + "','" + maHD + "')";
                ConnectFun.RunQuery(query);
                if (maKH != "")
                {
